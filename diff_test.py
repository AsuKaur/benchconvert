...
def differential_test_onnx_smt_c(onnx_model_path, smt_file_path, vnnlib_file_path, c_file_path, num_tests=100, tolerance=1e-5, verbose=True):
    ...
    #     verbose (bool): Whether to print detailed test output
...
def save_single_model_results(model_name, results, col_rename_map=None):
    # Save single model results to Excel with optional column renaming.
    # Prepare DataFrame from test results
    df = pd.DataFrame(results['tests'])
    
    # Convert boolean columns to uppercase strings
    boolean_cols = ['outputs_match', 'constraints_satisfied', 'passed']
...
def save_all_models_results(all_results, col_rename_map=None):
    # Save all models results to Excel with optional column renaming.
    file_name = "results/diff_test_results.xlsx"
    with pd.ExcelWriter(file_name) as writer:
        summary_data = []
        for model_name, results in all_results.items():
            ...
            summary_data.append({
                ...
                'passed_tests': passed_tests,
                'total_tests': total_tests,
                'test_id': results.get('test_id', 'N/A')
            })
...
def test_single_model(model_name, num_tests=100, tolerance=1e-5, col_rename_map=None):
    # Test a single model by name
    print(f"Testing single model: {model_name}")
    
    # Construct file paths based on model name
    onnx_model_path = f"onnx/{model_name}.onnx"
    smt_file_path = f"smt/{model_name}.smt2"
    vnnlib_file_path = f"vnnlib/{model_name}.vnnlib"
    c_file_path = f"c/{model_name}.c"
    
    # Check if all required files exist
    missing_files = []
    if not os.path.exists(onnx_model_path):
        missing_files.append(onnx_model_path)
    if not os.path.exists(smt_file_path):
        missing_files.append(smt_file_path)
    if not os.path.exists(vnnlib_file_path):
        missing_files.append(vnnlib_file_path)
    if not os.path.exists(c_file_path):
        missing_files.append(c_file_path)
    
    if missing_files:
        print(f"ERROR: Missing files for model {model_name}:")
        for file in missing_files:
            print(f"  - {file}")
        return None
    
    # Run differential testing
    results = differential_test_onnx_smt_c(
        onnx_model_path, 
        smt_file_path, 
        vnnlib_file_path, 
        c_file_path, 
    ...
    # Save results with optional renaming
    save_single_model_results(model_name, results, col_rename_map)
    
    return results

def test_all_models_in_folder(onnx_folder, num_tests=100, tolerance=1e-5, col_rename_map=None):
    # Test all models in the specified folder
    # Find all ONNX files in the specified folder
    onnx_files = glob.glob(os.path.join(onnx_folder, "*.onnx"))
    
    if not onnx_files:
        print(f"No ONNX files found in {onnx_folder}")
        return None
    
    print(f"Found {len(onnx_files)} ONNX files to test in {onnx_folder}")
    ...
    for onnx_file in sorted(onnx_files):
        ...
        if missing_files:
            print(f"SKIPPING {model_name}: Missing files - {', '.join(missing_files)}")
            skipped_models += 1
            continue
        ...
        try:
            ...
            overall_passed = overall_passed and results['all_passed']
            tested_models += 1
            
            # Print immediate result for this model
            passed_tests = sum(1 for test in results.get('tests', []) if test.get('passed', False))
            total_tests = len(results.get('tests', []))
            status = 'PASS' if results['all_passed'] else 'FAIL'
            print(f"\nModel {model_name}: {status} ({passed_tests}/{total_tests} tests passed)")
    ...
    if all_results:
        ...
        for model_name, results in all_results.items():
            ...
            if not results['all_passed']:
                failed_tests = [test for test in results.get('tests', []) if not test.get('passed', True)]
                print(f"  Failed tests: {len(failed_tests)}")
                if failed_tests:
                    print("  Failure reasons:")
                    for test in failed_tests[:3]:  # Show first 3 failures
                        if 'error' in test:
                            print(f"    Test {test['test_number']}: {test['error']}")
                        elif not test.get('outputs_match', True):
                            print(f"    Test {test['test_number']}: Output mismatch")
                        elif not test.get('constraints_satisfied', True):
                            print(f"    Test {test['test_number']}: Constraint violation")
            print()
    
    print("\n")
    
    # Save all results with optional renaming
    save_all_models_results(all_results, col_rename_map)
    
    return all_results

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run differential testing on ONNX, SMT, and C models.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Test a single model by name
  python diff_test.py unsat_v2_c3
  
  # Test all models in a folder
  python diff_test.py --folder models/
  
  # Test with custom parameters
  python diff_test.py --folder models/ --num-tests 50 --tolerance 1e-6
        """
    )
    
    # Create mutually exclusive group for single model vs folder
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('model_name', nargs='?', help='Name of a single model to test (e.g., unsat_v2_c3)')
    group.add_argument('--folder', help='Path to folder containing ONNX files to test all models')
    
    parser.add_argument('--num-tests', type=int, default=100, help='Number of test cases per model (default: 100)')
    parser.add_argument('--tolerance', type=float, default=1e-5, help='Tolerance for output comparison (default: 1e-5)')
    ...
    col_rename_map = {
        ...
        'c_outputs': 'C Outputs',
        'onnx_smt_diff': 'ONNX-SMT Differences',
        'onnx_c_diff': 'ONNX-C Differences',
        'smt_c_diff': 'SMT-C Differences',
        'outputs_match': 'Outputs Match',
        'constraints_satisfied': 'Constraints Satisfied',
        'passed': 'Test Passed',
        'error': 'Error',
        'smt_sat': 'SMT SAT Result'
    }

    if args.model_name:
        # Single model testing
        test_single_model(args.model_name, args.num_tests, args.tolerance, col_rename_map)
    elif args.folder:
        # Batch testing
        test_all_models_in_folder(args.folder, args.num_tests, args.tolerance, col_rename_map)
